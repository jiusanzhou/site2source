/**
 * 同构 md5 — Node 用 crypto，浏览器用纯 JS 实现
 * 因为 sign-verifier 需要在扩展 UI 和 Node 测试里都能跑
 */

// 纯 JS MD5 (基于经典 RFC 1321 实现, 支持 UTF-8)
// 从 blueimp-md5 简化: https://github.com/blueimp/JavaScript-MD5
function md5Pure(str: string): string {
  ...
  function md51(s: string) {
    ...
    let i;
    for (i = 64; i <= n; i += 64) md5cycle(state, md5blks(s.substring(i - 64, i)));
    s = s.substring(i - 64);
  ...
  function rhex(n: number) { const hex_chr = "0123456789abcdef"; let s = ""; for (let j = 0; j < 4; j++) s += hex_chr.charAt((n >> (j * 8 + 4)) & 0x0f) + hex_chr.charAt((n >> (j * 8)) & 0x0f); return s; }
